document.addEventListener('DOMContentLoaded', () => {
    ...
    translateBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (translationComplete) {
                // Add download button
                addDownloadButton(currentFile);
                
                // If summary was generated, add summary download button
                if (summaryGenerated) {
                    addSummaryDownloadButton(currentFile);
                }
            }
            
            addLog('Translation process finished');
        } catch (error) {
    ...
    function handleProgressUpdate(data) {
        ...
        if (data.summary) {
            ...
            addLog('Document summary received and displayed.');
        }
    }
    ...
    function addDownloadButton(filename) {
        ...
        downloadBtn.onclick = function() {
            // Remove the original extension and add 'download_' prefix
            const baseName = filename.replace(/\.[^/.]+$/, "");
            window.location.href = `/download/${encodeURIComponent(baseName)}`;
